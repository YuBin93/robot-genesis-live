// public/js/app.js (V4.3 - Final Stability Fix)

class RobotGenesisAppV4 {
    constructor() {
        this.container = document.getElementById('app-container');
        if (!this.container) {
            console.error("Fatal Error: #app-container not found. App cannot start.");
            return;
        }
    ...
    renderInitialView() {
        ...
        `;
        document.getElementById('search-btn')?.addEventListener('click', () => this.performAnalysis());
        document.getElementById('robotInput')?.addEventListener('keypress', (e) => { if (e.key === 'Enter') this.performAnalysis(); });
    }

    async performAnalysis() {
        const input = document.getElementById('robotInput')?.value.trim();
        const reportContainer = document.getElementById('report-container');
        if (!input || !reportContainer) return;
    ...
    renderTechSection(t2) {
        ...
        if (mappings.length === 0) { html += `<p>未能从信息源中解析出详细的技术与硬件映射关系。</p>`; }
        else {
    ...
    renderSupplySection(t3) {
        ...
        if (mappings.length === 0) { html += `<p>未能从信息源中解析出详细的供应商信息。</p>`; }
        else {
            mappings.forEach(m => {
                html += `<h4>${m?.hardware ?? '未知硬件'}</h4><ul>`;
                (m?.suppliers ?? []).forEach(s => { html += `<li>${s?.name ?? '未知供应商'} (${s?.country ?? '地区未知'})</li>`; });
                html += '</ul>';
            });
        }
        html += `</div>`;
    ...
    renderMarketSection(t4) {
        const summaryText = t4?.market_analysis_summary ?? '未能生成市场分析摘要。';
        return `<div id="section-market" class="report-section card"><h2>市场格局分析</h2><p style="white-space: pre-wrap;">${typeof summaryText === 'object' ? JSON.stringify(summaryText, null, 2) : summaryText}</p></div>`;
    }
    ...
    drawSankeyDiagram(sankeyData) {
        ...
        const height = Math.max(500, nodes.length * 25);
        ...
        svg.append("g").selectAll("rect").data(sankeyNodes).join("rect").attr("x", d => d.x0).attr("y", d => d.y0).attr("height", d => d.y1 - d.y0).attr("width", d => d.x1 - d.x0).attr("fill", d => color(d.id.split(' ')[0])).append("title").text(d => `${d.id}\n${d.value}`);
        const link = svg.append("g").attr("fill", "none").attr("stroke-opacity", 0.6).selectAll("path").data(sankeyLinks).join("path").attr("d", d3.sankeyLinkHorizontal()).attr("stroke", d => color(d.source.id.split(' ')[0])).attr("stroke-width", d => Math.max(1, d.width));
        link.append("title").text(d => `${d.source.id} → ${d.target.id}\nValue: ${d.value}`);
        svg.append("g").style("font", "12px sans-serif").attr("fill", "#fff").selectAll("text").data(sankeyNodes).join("text").attr("x", d => d.x0 < width / 2 ? d.x1 + 6 : d.x0 - 6).attr("y", d => (d.y1 + d.y0) / 2).attr("dy", "0.35em").attr("text-anchor", d => d.x0 < width / 2 ? "start" : "end").text(d => d.id);
    }
}

document.addEventListener('DOMContentLoaded', () => {
    if (typeof d3 === 'undefined') {
        document.body.innerHTML = '<div style="color:red; text-align:center; padding-top: 20vh;">Fatal Error: D3.js library failed to load. Please check your internet connection and refresh.</div>';
        return;
    }
